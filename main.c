...
int main(){
    ...
    //misc vars
    int index;
    float light[3] = {3.0, 5.0, -15.0};
    float missColor[3] = {0.0, 0.0, 0.0};
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            if (hit) {
                ...
                setColorAtIndex(image, getPixColor(keeper.mat.color, diffuse), index);
            } else {
                setColorAtIndex(image, missColor, index);
            }
        }
    }

    //write image to file and free mem
    stbi_write_png("reference.png", width, height, chan, image, width*chan);
    free(image);
